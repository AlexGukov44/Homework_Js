...
cylinder.add(sound);  //  привязка звука к положению фигуры в пространстве 
...
function animate() {
    ...
    if (keys['w']) sphere.position.z -= 0.05;   // вперед  у - тогда вверх
    if (keys['s']) sphere.position.z += 0.05;   // назад   у - тогда вниз
    if (keys['a']) sphere.position.x -= 0.05;   // влево
    if (keys['d']) sphere.position.x += 0.05;   // вправо
